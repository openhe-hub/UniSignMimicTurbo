...
def graft_pose(ref_pose: dict, video_pose: dict) -> dict | None:
...
def graft_pose_v2(ref_pose: dict, video_pose: dict) -> dict | None:
    NECK_ID = 1
    R_SHOULDER_ID = 2
    R_ELBOW_ID = 3
    R_WRIST_ID = 4
...
def graft_pose_v3(ref_pose: Dict, video_pose: Dict, transform_smoother: Optional[TransformSmoother] = None) -> Optional[Dict]:
    """
...
def solve_similarity_transform(source_points: np.ndarray, target_points: np.ndarray) -> Tuple[np.ndarray, float, np.ndarray]:
    """
...
def align_naive(ref_kpts, result_animation):
    # print(ref_kpts.shape, result_animation.shape)
