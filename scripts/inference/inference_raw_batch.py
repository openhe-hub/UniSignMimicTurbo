...
def main(args):
    ...
    for video in all_videos:
        ...
        task.num_frames = min(int(len(pose_pixels) / 16) * 16 - 1, 199)
        logger.info(f"using num video frame = {len(pose_pixels)}, num frame = {task.num_frames}")
        ...
        save_to_mp4(
            _video_frames, 
            f"{args.output_dir}/{infer_config.batch.video_folder.split('/')[-1]}/{os.path.basename(task.ref_video_path).split('.')[0]}" \
...
if __name__ == "__main__":    
    ...
    parser.add_argument("--batch_folder", type=str, default="", help="batch folder")
