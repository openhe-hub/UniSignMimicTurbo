...
from mimicmotion.dwpose.preprocess2 import get_video_pose, get_image_pose
...
def main(args):
    ...
    for task in infer_config.test_case:
        logger.info(f"handling: video = {task.ref_video_path}, img = {task.ref_image_path}")
        ...
        ) # [N_frame, 3, W, H], [1, 3, W, H]
        
        # Export pose_pixels and image_pixels tensors
        video_name = os.path.basename(task.ref_video_path).split('.')[0]
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
        
        # Save pose_pixels as video
        # Convert from normalized [-1, 1] back to [0, 255]
        pose_video_frames = ((pose_pixels + 1.0) * 127.5).clamp(0, 255).to(torch.uint8)
        
        # Remove the first frame (reference image pose) to get only video poses
        pose_video_frames = pose_video_frames[1:]  # Shape: [N_frames-1, 3, H, W]
        
        # Save as video
        pose_video_path = f"{args.output_dir}/{video_name}_pose_video_{timestamp}.mp4"
        save_to_mp4(
            pose_video_frames,
            pose_video_path,
            fps=task.fps if hasattr(task, 'fps') else 30,
        )
        logger.info(f"Saved pose video to {pose_video_path}")
        
        # # Also save reference image pose as a single image
        # ref_pose_frame = ((pose_pixels[0] + 1.0) * 127.5).clamp(0, 255).to(torch.uint8).numpy()
        # ref_pose_path = f"{args.output_dir}/{video_name}_ref_pose_{timestamp}.png"
        # import cv2
        # # Convert from CHW to HWC and RGB to BGR for cv2
        # ref_pose_bgr = cv2.cvtColor(ref_pose_frame.transpose(1, 2, 0), cv2.COLOR_RGB2BGR)
        # cv2.imwrite(ref_pose_path, ref_pose_bgr)
        # logger.info(f"Saved reference pose image to {ref_pose_path}")
