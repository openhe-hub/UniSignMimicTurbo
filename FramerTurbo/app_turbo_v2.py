...
from models_diffusers.lcm_scheduler import AnimateLCMSVDStochasticIterativeScheduler
...
SCHEDULER_CHOICES = ("euler", "dpm++", "lcm")
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Folder containing pairs of images named {id}_start.jpg and {id}_end.jpg.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Motion bucket id for the diffusion pipeline.",
    )
    ...
    parser.add_argument(
        ...
        default="dpm++",
        help="Noise scheduler: euler(30 steps), dpm++(15-20 steps), lcm(4-6 steps)",
    )
    parser.add_argument("--num_inference_steps", type=int, default=None,
                       help="Number of inference steps (auto-selected based on scheduler if not specified)")

    # LCM specific parameters
    parser.add_argument("--lcm_num_train_timesteps", type=int, default=40)
    ...
    parser.add_argument("--scheduler_clip_denoised", action="store_true",
    ...
    parser.add_argument("--dpm_solver_order", type=int, default=2, choices=[1, 2, 3],
                       help="Order of DPM-Solver (higher = better quality but slower)")
    parser.add_argument("--dpm_use_karras_sigmas", action="store_true",
                       help="Use Karras sigmas for DPM-Solver++")

    args = parser.parse_args()

    # Set default num_inference_steps based on scheduler
    if args.num_inference_steps is None:
        if args.scheduler == "lcm":
            args.num_inference_steps = 6  # Conservative LCM setting
        elif args.scheduler == "dpm++":
            args.num_inference_steps = 15  # DPM++ 避免索引越界 (原20步会出错)
        else:
            args.num_inference_steps = 30  # Original Euler setting

    return args


def configure_scheduler(pipeline, args):
    """Configure scheduler based on args"""
    if args.scheduler == "euler":
        print(f"Using EulerDiscreteScheduler with {args.num_inference_steps} steps")
    ...
    if args.scheduler == "dpm++":
        scheduler = DPMSolverMultistepScheduler.from_config(
            ...
            lower_order_final=True,  # 修复：避免最后一步索引越界
        )
        pipeline.scheduler = scheduler
        print(
            f"Swapped to DPMSolverMultistep++ scheduler with {args.num_inference_steps} steps "
            f"(order={args.dpm_solver_order}, karras_sigmas={args.dpm_use_karras_sigmas})"
        )
        return scheduler
    ...
    if args.scheduler == "lcm":
        ...
        pipeline.scheduler = scheduler
        print(
            f"Swapped to AnimateLCM_SVD scheduler with {args.num_inference_steps} steps "
            f"(train_steps={args.lcm_num_train_timesteps}, sigma_min={args.scheduler_sigma_min:.4f}, "
            f"sigma_max={args.scheduler_sigma_max:.1f})"
        )
        return scheduler
...
def save_gifs_side_by_side(
    ...
    base_dir = output_folder if os.path.isdir(output_folder) else os.path.dirname(output_folder)
    if base_dir == "":
        base_dir = "."
    ensure_dirname(base_dir)

    for idx, image_list in enumerate([validation_control_images, flattened_batch_output]):

        gif_path = os.path.join(base_dir, f"temp_{idx}_{timestamp}.gif")
...
class Drag:
    ...
    def run(self, first_frame_path, last_frame_path, tracking_points, controlnet_cond_scale, motion_bucket_id, output_path=None):
        ...
        if hasattr(tracking_points, "constructor_args"):
        ...
        if (len(input_all_points) == 0) and self.use_sift:
            ...
            output_file_sift = os.path.join(self.args.output_dir, "sift.png")
        ...
        video_frames = self.pipeline(
            ...
            fps=7,
            num_inference_steps=self.args.num_inference_steps,  # Use configurable steps
            # track
        ...
        if output_path is None:
            val_save_dir = os.path.join(self.args.output_dir, "vis_gif.gif")
        else:
            val_save_dir = output_path
        ...
        return val_save_dir, inference_time
...
def run_framer_wrapper(first_frame_path, last_frame_path, tracking_points, controlnet_cond_scale, motion_bucket_id):
    """Wrapper for Gradio that discards the inference_time return value"""
    gif_path, inference_time = Framer.run(first_frame_path, last_frame_path, tracking_points, controlnet_cond_scale, motion_bucket_id)
...
if __name__ == "__main__":
    ...
    # Scheduler name mapping for display
    scheduler_display = {
        "euler": "Euler (30 steps, best quality)",
        "dpm++": f"DPM++ ({args.num_inference_steps} steps, balanced)",
        "lcm": "LCM (6 steps, fastest)"
    }

    with gr.Blocks() as demo:
        gr.Markdown("""<h1 align="center">Framer Turbo V2: Interactive Frame Interpolation with Multiple Schedulers</h1><br>""")

        gr.Markdown(f"""Gradio Demo for <a href='https://arxiv.org/abs/2410.18978'><b>Framer: Interactive Frame Interpolation</b></a> with Advanced Schedulers.<br>
                    Github Repo can be found at https://github.com/aim-uofa/Framer<br>
                    <b>Current Mode: {scheduler_display.get(args.scheduler, args.scheduler)}</b><br>
        ...
        gr.Markdown(f"""## Usage: <br>
                    ...
                    <br>
                    <b>Scheduler Options:</b><br>
                    &ensp; • <b>Euler</b>: 30 steps - Best quality, slower (original)<br>
                    &ensp; • <b>DPM++</b>: 15-20 steps - Balanced quality/speed (recommended)<br>
                    &ensp; • <b>LCM</b>: 4-6 steps - Fastest, good quality<br>
                    <br>
                    <b>Current: {args.scheduler.upper()} with {args.num_inference_steps} steps</b><br>""")

        # device, args, height, width, model_length
        Framer = Drag("cuda", args, 320, 512, 14, use_sift=bool(getattr(args, "use_sift", 1)))
        first_frame_path = gr.State()
        last_frame_path = gr.State()
        tracking_points = gr.State([])
        ...
        run_button.click(run_framer_wrapper, [first_frame_path, last_frame_path, tracking_points, controlnet_cond_scale, motion_bucket_id], [output_video])
